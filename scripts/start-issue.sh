...
#
# worktreeが既にある場合は作り直さず再利用する。一度閉じたセッションに戻るための経路
...
WORKTREE_BASE="${DAYSPAN_WORKTREE_BASE:-$HOME/apps/dayspan-worktrees}"
PROMPT_TEMPLATE="$ROOT/scripts/prompts/implementation-agent.md"
PROMPT_DIR="$WORKTREE_BASE/.prompts"

# ワンクリック起動（issue-deck側の scripts/start-local-session.sh）から呼ばれた場合に立つ。
# LANアクセス設定はWindowsの管理者権限を要求し、wt.exeで開いたタブではUACを承認しても
# 待ちから戻らずタブが固まるため、この経路では行わない。
...
# タブは非対話シェルで始まり、nvmを ~/.bashrc に置いていると読まれない。
...
prepare_issue() {
  ...
  # 既存のworktreeは作り直さず再利用する。ただしworktreeとして壊れている場合や
  ...
  local issue_json
  if ! issue_json="$(gh issue view "$n" --repo guchi-apps/dayspan --json number,title,body,labels,comments)"; then
  ...
  if [[ "$SKIP_LAN_SETUP" != "0" ]]; then
    # ワンクリック起動経路。UACを承認しても待ちから戻らずタブが固まるため行わない。
