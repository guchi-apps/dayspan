...
# ポートを掴んでいることがある。再開時にpnpm devが起動できなくなるため先に止める。
...
# 「8. Claude Codeへのコンテキストの渡し方」。issue-deckリポジトリのドキュメント）。
# cloneしていない環境でも起動できるよう、
# 存在しない場合は --add-dir を付けずにそのまま起動する。
SHARED_CONTEXT_DIR="${DAYSPAN_SHARED_CONTEXT_DIR:-$HOME/apps/_docs}"
CLAUDE_EXTRA_ARGS=()
