...
/**
 * 年休の付与日数・年度の開始月・1日の所定労働時間を保存する（docs/spec.md §34）。
 *
 * 年度ごとの付与（`AnnualLeaveGrant`）と全年度共通の設定（`UiSetting.fiscalYearStartMonth`・
 * `UiSetting.workMinutesPerDay`）をひとつの `PUT` で受ける。画面ではどれも同じダイアログで
 * 直すため、分けると保存の往復が増える。
 *
...
export async function PUT(request: Request) {
  ...
  const { fiscalYear, grantedDays, carriedOverDays, fiscalYearStartMonth, workMinutesPerDay } =
    body;
  ...
  // 60分（1時間）を割る所定は勤務の記録として意味をなさず、1440分（24時間）を超える値も
  // 打ち間違いでしかない。時間休の上限（所定 − 1時間）を決める値なので、ここで範囲を切る。
  if (
    workMinutesPerDay !== undefined &&
    (!Number.isInteger(workMinutesPerDay) || workMinutesPerDay < 60 || workMinutesPerDay > 1440)
  ) {
    return NextResponse.json(
      {
        error: "invalid_work_minutes",
        message: "1日の所定労働時間は1時間〜24時間の範囲で入力してください。",
      },
      { status: 400 },
    );
  }

  // 送られてきた項目だけを書く。片方だけを直したときに、もう片方を既定値へ戻さないため。
  const uiSettingFields = {
    ...(fiscalYearStartMonth === undefined ? {} : { fiscalYearStartMonth }),
    ...(workMinutesPerDay === undefined ? {} : { workMinutesPerDay }),
  };
  ...
  await Promise.all([
    ...
    fiscalYearStartMonth === undefined && workMinutesPerDay === undefined
      ? Promise.resolve(null)
      : db.uiSetting.upsert({
          where: { userId },
          create: { userId, ...uiSettingFields },
          update: uiSettingFields,
        }),
  ]);
