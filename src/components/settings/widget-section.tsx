...
export function WidgetSection({
   ...
   * `bridge` はHTTPSの受け渡しページ（既定）、`app` は `webapp://` を直接開く従来の値。
   * どちらも http のアドレスで開いているときは null（渡す相手が存在しないため）。
   */
  openUrls: { bridge: string | null; app: string | null; browser: string };
  ...
  //
  // httpのアドレスで開いているときは受け渡しページも作られない（渡す相手が存在しないため）。
  // 挟んでもブラウザで開くのと同じところへ着くので、従来どおりブラウザで開くURLを案内する。
  //
  // constへ受けるのは、押されたときに走るコールバックの中で型の絞り込みが効かなくなるため
  // （プロパティのままだと非nullアサーションが要る）。
  const bridgeUrl = openUrls.bridge;
  const directAppUrl = openUrls.app;
  const openUrl = bridgeUrl ?? openUrls.browser;
  const openUrlTarget: CopyTarget = bridgeUrl ? "bridgeUrl" : "browserUrl";
                      ...
                      {directAppUrl && (
                        <p className="flex flex-wrap items-center gap-2">
                          <span>
                            ...
                            に変えてください。直接開ける端末では、こちらのほうが速く開き、Safariに
                            受け渡しページのタブも残りません。
                          </span>
                          <Button
                            variant="outline"
