 ...
 * スマートフォンの月表示（チップに残るのは約22px）で2文字目が数pxだけ顔を出した形で切れ、
 ...
 * 幅390pxの端末の月表示で1文字しか入らず、「大」だけでは行き先も勤務場所も読めないため。
 * 高さ（`leading-[15px]`）は変えないので、日付の見出しの高さは動かない。
 */
 ...
 * | 12px未満 | 色だけ | 色だけ |
 *
 * 幅390pxの端末の月表示でチップに残るのは約22px で、8pxの文字が2つ入る。判定は画面幅では
 * なく**チップに残っている幅**で行う（列の幅は表示形式と画面幅の掛け算で決まるため、
 * ブレークポイントで切るとPCの月表示のような幅のある列でも文字が縮む・docs/spec.md §34）。
 *
 * 12px未満になるのは、月表示のうち月の変わり目の日（数字が `9/1` のように広がる）くらい。
 */
