...
async function loadSource(userId: string, now: Date): Promise<TasksSource> {
  const cached = readWidgetCache<TaskItem[]>(userId, "tasks", now);
  if (cached) return { ok: true, tasks: cached };

  const connection = await getNotionConnection(userId);
  if (!connection?.taskDataSourceId) return { ok: false, unavailable: "notion_not_connected" };
  ...
  writeWidgetCache(userId, "tasks", tasks, now);
 ...
 * 過ぎたのか半年放置しているのかは分からないため、タスク画面と同じ言葉を使う。
 */
function dueLabelOf(
  task: TaskItem,
  ...
  // これからの期限は年を出さない。枠の幅がそのぶん項目名から削られ、年をまたぐ期限は
  // ウィジェットに並ぶほど近くない。
