...
| `/api/widget/` は proxy の中でSupabaseを通さず素通しする | 認証がトークンで完結しており、更新のたびにSupabase Authへ往復させる理由が無い。Supabaseへ到達できない間、トークンだけで判定できる要求まで503になるのも避ける。matcherから外さないのは、外すと詐称されたユーザーIDヘッダーが後段へ届くため |
