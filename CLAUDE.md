...
| ゴミの日の印はゴミ箱の線画にし、絵文字は使わない | 日付リマインドと同じ菱形だと、「普通ごみ」「資源ごみ」といった品目名を読むまでゴミの日だと分からない。週に2〜3件入るため読む手間がそのぶん積み上がる。絵文字（🗑️）は端末のフォント次第で色も形も変わり、印の並びで1つだけ多色になるうえ、ダークテーマでの見え方をこちらから直せない。lucideの線画なら他のアイコンと同じ描き方で、枠線と同じ tertiary に乗る。品目ごとには分けない（Notionへ渡るのは品目名の文字列だけ） |
