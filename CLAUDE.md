...
| ウィジェットはブラウザではなくホーム画面のDaySpanを開く | iOSは `https://` のリンクを必ずSafariで開き、ホーム画面のWebアプリへ渡さない。SafariとWebアプリはストレージが別扱いのため、Safariへ落ちるとログインし直しになることがある。Webアプリを開けるのは `webapp://<ホスト>` スキーム（iOS 16.4以降）だけ。ただしパスは無視され最初の画面（`start_url` = `/calendar`）から開くため、記録画面へ直接は入れない。ホーム画面に追加していない端末では開けないので、台本の先頭の `OPEN_IN` でブラウザへ切り替えられるようにする。`http` のアドレスで作った台本は `webapp://` を組み立てない（httpはWebアプリとして追加できない） |
