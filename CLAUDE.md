...
| 記録中の帯は月表示に描かない | 月表示の週ごとの配置は、予定・タスクが変わったときだけ組み直している。毎分伸びる帯を混ぜると記録中はその計算が毎分やり直しになる。記録中かどうかと経過時間は、どの表示形式でも記録ボタンのピルが出している |
| 記録の開始・終了の時刻はサーバーの時計で決める | 端末の時計がずれていると、記録した時間帯そのものがずれた予定になる。押し忘れの修正は開始時刻を直す経路で行う |
| 記録ボタンはオフラインでも隠さず、押せない状態にする | 名前と経過時間を出す表示でもあるため。押せなければ意味の無い「＋」とは違い、残しておく意味がある |
